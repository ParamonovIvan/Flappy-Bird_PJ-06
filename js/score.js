import Canvas from "./canvas.js";

export default class Score {
  constructor() {
    this.canvas = new Canvas();

    this._score = 0;
    this._bestScore = 0;

    this.positionX;
    this.positionY = 70;

    this.audioPoint = new Audio();
    this.audioPoint.src = "./sounds/point.wav";

    this.audioPoint10 = new Audio();
    this.audioPoint10.src = "./sounds/point-10.wav";
  };

  increaseScore() {
    this._score++;
  };

  bestScoreRecord() {
    if (this._score > this._bestScore) {
      localStorage.setItem('bestScore', this._score);
    }
    this._bestScore = Number(localStorage.getItem('bestScore'));
  };

  localStorageScore() {
    if (localStorage.getItem('bestScore')) {
      this._bestScore = Number(localStorage.getItem('bestScore'));
    } else {
      this._bestScore = 0;
    }
  };

  draw() {
    if (this._score < 10) {
      this.positionX = this.canvas.element.width / 2 - 15;
      } else {
        this.positionX = this.canvas.element.width / 2 - 35;
      }

    this.canvas.context.font = '32px PressStart2P';
    this.canvas.context.lineWidth = 2;

    this.canvas.context.fillStyle = '#AFEEEE';
    this.canvas.context.fillText(this._score, this.positionX, this.positionY);

    this.canvas.context.strokeStyle = '#000000';
    this.canvas.context.strokeText(this._score, this.positionX, this.positionY);
  }
}